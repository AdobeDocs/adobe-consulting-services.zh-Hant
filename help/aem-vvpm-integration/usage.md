...
1. 將下列專案新增至專案中的`.cloudmanager/maven/settings.xml`檔案或電腦上的`~/.m2/settings.xml`。 以使用者名稱取代`YOUR_USERNAME`，並以入門電子郵件中提供的密碼取代`YOUR_PASSWORD`。
...
3. 將下列專案新增至專案的`all/pom.xml`檔案。 以適當的版本取代`project.dependencies.dependency.version`，以正確的路徑取代`project.build.plugins.plugin.configuration.embeddeds.embedded.target`。
...
1. 導覽至Veeva雲端設定。
   ...
   ![瀏覽至雲端設定](assets/cloud-config-navigate.png)
...
2. 必填。 Veeva執行個體的網域URL （例如`https://my-instance.veevavault.com/`）
3. 必填。 呼叫Veeva Vault API所需的ClientID。 此值可為任意值，主要用於偵錯。 （例如`adobe-aem-vvtechpartner`）
4. 必填。 Veeva儲存庫使用者名稱。 請參閱[Veeva使用者建立](#veeva-user-creation)。
5. 必填。 Veeva儲存庫密碼。 請參閱[Veeva使用者建立](#veeva-user-creation)。
...
2. 必填。 產生頁面影像的動作名稱。 此值必須是`aem-veeva-integration/get-image-async`。
3. 必填。 用於產生html影像的動作名稱。 此值必須是`aem-veeva-integration/get-pdf-async-new`。
...
5. 必填。 AdobeIO使用的AEM使用者名稱。 請參閱[AEM使用者建立](#aem-user-creation)。
6. 必填。 AdobeIO使用的AEM密碼。 請參閱[AEM使用者建立](#aem-user-creation)。
